...
| --- | --- | --- |
| `router/tasks.py` | 提供触发关键词与主题分析和查看结果的 HTTP 入口 | 写提取和归类规则细节 |
| `services/preprocess_service.py` | 提供有效标准样本读取能力 | 直接做关键词和主题判断 |
| `services/sentiment_service.py` | 提供可复用的情感结果契约 | 直接承担 Day 7 主链 |
| `services/topic_service.py` | 编排关键词提取、主题归类、任务级聚合 | 直接写 SQL |
| `shcemas/topic_schema.py` | 定义样本级关键词 / 主题结果和任务级摘要 | 持有业务流程 |
| `crud/task_crud.py` | 回写关键词排行、主题分布和分析状态 | 直接做提取与归类 |
| `models/task_model.py` | 为 Day 7 的关键词和主题结果留持久字段 | 承担关键词 / 主题判断 |
...
class TopicService:
    ...
    def classify_topic(self, sample: NormalizedSample, keywords: list[str]):
        # 你要做的事：
        # 1. 根据文本和关键词判断主题
        # 2. 给出一个主题标签
        # 3. 给出一个极简归类原因
...
class TopicService:
    def extract_keywords(self, sample: NormalizedSample) -> tuple[list[str], list[str]]:
...
2. 这里先用最小可解释规则示例，是为了把关键词与主题结果契约和聚合主链立住  
3. 后面即使替换成更强的提取 / 归类方式，`SampleTopicResult`、`TaskKeywordSummary`、`TaskTopicSummary` 这层边界仍然应该稳定  
