...
- 先识别物流、质量、价格、服务、功能等最小问题类别，不追求复杂因果推理
...
flowchart LR
    ...
    B --> C[run or load sentiment results]
...
| `services/sentiment_service.py` | 提供样本情感标签和分数 | 挑代表样本 |
...
2. 今天先用规则归因，是为了让链路可解释、可验证、可替换  
3. 后面即使用 LLM 或图谱增强，`SampleIssueAttribution` 和 `IssueAttributionResponse` 仍然应该稳定  
