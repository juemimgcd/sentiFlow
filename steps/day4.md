...
| `services/task_service.py` | 内容抽取编排、元信息提取、dataset/task 创建 | HTTP 协议处理 |
| `MarkItDown adapter` | 负责 PDF / Office / HTML 等文件转 Markdown / 文本 | 任务状态流转 |
| `pipelines/` | 今天先不启用 | 今天不承接 Day 4 主线 |
| `crud/` / `models/` | 今天可暂不接入 | 不抢 Day 4 的主链焦点 |
...
| 小改接入 | `shcemas/` `services/` | 新增 Day 4 专属 schema / service 文件 |
| 暂时不动 | `crud/` `models/` `pipelines/` `agent/` | 先保留，不在 Day 4 扩写 |
...
- 后面再决定要不要把它单独升级成 `file_ingest_service.py`

Day 4 先在 `task_service.py` 内部把这条边界讲清楚，就已经够用了。
...
### 今天为什么先不急着扩到 `crud/` / `models/` / `pipelines/`

因为当前仓库这些位置还没有真正承接起来。  
Day 4 最稳的做法，是先让接口边界和 service 职责跑顺。
...
### 步骤 3：在 `services/task_service.py` 里形成 adapter 分流
...
如果后面觉得 `task_service.py` 里的文件接入职责变厚，  
再进一步拆成：

- `services/file_ingest_service.py`
- `clients/markitdown_client.py`
...
3. Day 5 会接的是 dataset 中的标准化样本，而不是原始上传动作本身
...
## 14:40 - 15:30：在 `services/task_service.py` 里先把最小主链接起来
...
class TaskService:
    async def import_dataset(self, file: UploadFile, metadata):
        # 你要做的事：
        # 1. 校验文件后缀是否在白名单
        # 2. 调用抽取逻辑
        # 3. 返回 dataset_id
...
class TaskService:
    ...
    async def create_task(self, payload: CreateTaskRequest) -> CreateTaskResponse:
        dataset = _DATASET_STORE.get(payload.dataset_id)
        if dataset is None:
            raise HTTPException(
        ...
        _TASK_STORE[task_id] = {
            ...
            "source_platform": dataset["metadata"]["source_platform"],
            "product_line": dataset["metadata"].get("product_line"),
            "created_at": now,
        }
...
1. Day 4 可以先用内存存储把主链跑通，不必今天就把 ORM 和仓储层做满  
2. `dataset_id` 的价值非常大，它把导入和任务创建自然分开了  
3. `TaskService` 里今天先形成“adapter -> normalized payload”的抽取边界  
4. `SUPPORTED_IMPORT_TYPES` 是“可支持格式”的工程边界，不是装了 MarkItDown 就无限制放开  
5. Day 5 仍然接的是统一后的 dataset，不把清洗和标准化提前写满
...
from shcemas.task_schema import CreateTaskRequest, ImportMetadata
...
async def import_task_data(
    ...
    data = await task_service.import_dataset(file=file, metadata=metadata)
    return success_response(data=data.model_dump(), message="dataset imported")


@router.post("")
async def create_task(payload: CreateTaskRequest):
    data = await task_service.create_task(payload)
    return success_response(data=data.model_dump(), message="task created")


@router.get("/{task_id}")
async def get_task_detail(task_id: str):
    data = await task_service.get_task_detail(task_id)
...
- `services/task_service.py` 的最小职责讲清楚
